...
int main()
{
    ...
    Ship ship = {};
    ship.directions[0] = V2(1,0);
    ship.directions[1] = V2(0,-1);
    ship.directions[2] = V2(-1,0);
    ship.directions[3] = V2(0,1);
    ship.dirIndex = 0;
    ship.pos = V2(0,0);
    ...
    for (int i = 0; i < commandCount; ++i)
    {
        ...
        switch (com->operation) {
            case 'F': {
                v2 shipDir = ship.directions[ship.dirIndex];
                offset = V2(shipDir.x * com->value, shipDir.y * com->value);
                ship.pos = V2(ship.pos.x + offset.x, ship.pos.y + offset.y);
            } break;
            case 'L': {
                int rotAmount = com->value / 90;
                ship.dirIndex = (ship.dirIndex - rotAmount) % ArrayCount(ship.directions);
            } break;
            case 'R': {
                int rotAmount = com->value / 90;
                ship.dirIndex = (ship.dirIndex + rotAmount) % ArrayCount(ship.directions);
            ...
            case 'W' : {
                ship.pos = V2(ship.pos.x - com->value, ship.pos.y);
            } break;
            case 'N' : {
                ship.pos = V2(ship.pos.x, ship.pos.y + com->value);
            } break;
            case 'S' : {
                ship.pos = V2(ship.pos.x, ship.pos.y - com->value);
            } break;
