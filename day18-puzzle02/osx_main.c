...
void removeElement(u64 *arr, int len, int index)
{
...
void removeOp(enum Operator *arr, int len, int index)
{
    for (int i = index; i < len - 1; i++)
    {
        arr[i] = arr[i + 1];
...
static u64 CalcStatement(Statement *statement) {
    ...
    // First find the additions
    for (int valueIndex=statement->valueCount-1; 
        valueIndex >= 1; 
        valueIndex--) {
        ...
        if (op == Add)
        {
            u64 a = statement->values[valueIndex-1];
            u64 b = statement->values[valueIndex];
            u64 sum = a + b;
            statement->values[valueIndex-1] = sum;
            // remove from array is easiest
            removeElement(statement->values, statement->valueCount, valueIndex);
            removeOp(statement->op, statement->valueCount, valueIndex);
            statement->valueCount--;
        }
